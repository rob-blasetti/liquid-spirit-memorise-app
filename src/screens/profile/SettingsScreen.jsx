...
const SettingsScreen = ({
  ...
  const [continueCardHidden, setContinueCardHiddenState] = useState(false);
  const [continueCardReady, setContinueCardReady] = useState(false);
  ...
  const supportsContinueCard = ['1', '2', '2b'].includes(String(profile?.grade || ''));
  ...
  useEffect(() => {
    let mounted = true;
    if (!supportsContinueCard) {
      setContinueCardHiddenState(false);
      setContinueCardReady(true);
      return () => {
        mounted = false;
      };
    }
    setContinueCardReady(false);
    const loadPreference = async () => {
      try {
        const hidden = await getLibraryContinueCardHidden(profileId);
        if (!mounted) return;
        setContinueCardHiddenState(hidden);
      } catch {
        if (!mounted) return;
        setContinueCardHiddenState(false);
      } finally {
  ...
  }, [profileId, supportsContinueCard]);
  ...
  const toggleContinueCardVisibility = async () => {
    const nextHidden = !continueCardHidden;
    setContinueCardHiddenState(nextHidden);
    try {
      await setLibraryContinueCardHidden(profileId, nextHidden);
    } catch {
      setContinueCardHiddenState(!nextHidden);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {supportsContinueCard && (
          <View style={styles.section}>
            <Text style={styles.sectionLabel}>Library</Text>
            <View style={styles.sectionCard}>
              <View style={styles.fieldGroup}>
                <Text style={styles.fieldTitle}>Continue card</Text>
                <Text style={styles.fieldDescription}>
                  Control whether the "Continue Where You Left Off" card appears in Library.
                </Text>
                <TouchableOpacity
                  style={[
                    styles.libraryToggleButton,
                    !continueCardHidden && styles.libraryToggleButtonActive,
                  ]}
                  onPress={toggleContinueCardVisibility}
                  disabled={!continueCardReady}
                  accessibilityRole="button"
                  accessibilityLabel="Toggle Library continue card visibility"
                >
                  <Text style={styles.libraryToggleButtonText}>
                    {continueCardHidden ? 'Show Continue Card Again' : 'Hide Continue Card'}
                  </Text>
                  {continueCardReady ? (
                    <Ionicons
                      name={continueCardHidden ? 'eye-outline' : 'eye-off-outline'}
                      size={20}
